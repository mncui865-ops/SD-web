...
النسخة النهائية v8.0
- عد تنازلي عند الإرسال والرد
- حذف تلقائي بعد يوم واحد
- توقيت الخرطوم
- منع الإرسال لنفسك
- إشعارات + صوت (بدون زر تفعيل/إلغاء)
- إزالة كشف اللقب والبروفايل (الكل مجهول)
- زر: تنزيل الصورة فقط (لا تعديل ولا حذف)
...
# ============================================================
# 🗑️ حذف تلقائي للرسائل بعد يوم واحد
# ============================================================
MESSAGE_TTL_DAYS = 1

def ttl_unit(n):
    """إرجاع الصيغة العربية الصحيحة للوحدة الزمنية"""
    if n == 1:
        return 'يوم'
    elif n == 2:
        return 'يومان'
...
<section class="section" style="padding-top:0;">
    <div class="container">
        <div class="live-stats">
            ...
            <div class="live-stat"><div class="ls-num">{{ message_ttl_days }}</div><div class="ls-lbl">{{ icons.hourglass|safe }} {{ ttl_unit(message_ttl_days) }} قبل الحذف</div></div>
        </div>
    </div>
</section>
    ...
    <div class="container">
        ...
        <div class="features-grid">
            ...
            <div class="feature-card">
                ...
                <h4>حذف تلقائي</h4>
                <p>الرسائل تُحذف تلقائياً بعد يوم واحد</p>
            </div>
            <div class="feature-card">
...
def auth_callback():
    ...
    if not username:
        ...
        add_notification(
            ...
            f'شارك رابطك لاستقبال الرسائل المجهولة. الرسائل تُحذف تلقائياً بعد {MESSAGE_TTL_DAYS} {ttl_unit(MESSAGE_TTL_DAYS)}.',
            link='/profile',
        )
    else:
        user = users_db[username]
...
def messages():
    ...
    for m in msgs:
        try:
            ...
            if created.tzinfo is None:
                created = created.replace(tzinfo=KHARTOUM_TZ)
            elapsed_seconds = (now_khartoum() - created).total_seconds()
            days_left = max(0, MESSAGE_TTL_DAYS - int(elapsed_seconds // 86400))
    ...
    <section class="section">
        <div class="container">
            ...
            <div class="alert alert-info show" style="text-align:center; margin-bottom:16px;">
                {{ icons.hourglass|safe }} الرسائل تُحذف تلقائياً بعد {{ message_ttl_days }} {{ ttl_unit(message_ttl_days) }} من إرسالها
            </div>
            <div class="share-box">
            ...
            {% if messages %}
                ...
                <div class="msg-card" id="msg-{{ m.id }}">
                    <div class="msg-ttl-badge">
                        {{ icons.hourglass|safe }} تُحذف بعد {{ m.days_left }} {{ ttl_unit(m.days_left) }}
                    </div>
    ...
    <section class="section">
        <div class="container" style="max-width:880px;">
            ...
            <div class="alert alert-warning show" style="text-align:center; margin-bottom:20px;">
                {{ icons.hourglass|safe }} كل رسالة تُحذف تلقائياً بعد {{ message_ttl_days }} {{ ttl_unit(message_ttl_days) }} من إرسالها
            </div>
                ...
                <div class="pub-card">
                    ...
                    <div class="pub-footer">
                        ...
                        <div class="pub-tags">
                            ...
                            <span class="pub-tag tag-ttl">{{ icons.hourglass|safe }} {{ m.days_left }} {{ ttl_unit(m.days_left) }} متبقي</span>
                            {% if m.replied %}
...
def _handle_send_message(username, user):
    ...
    <div class="form-wrapper" style="max-width:600px;">
        ...
        <div class="alert alert-info show" style="text-align:center; margin-bottom:16px;">
            {{ icons.hourglass|safe }} ستُحذف رسالتك تلقائياً بعد {{ message_ttl_days }} {{ ttl_unit(message_ttl_days) }}
        </div>
    ...
    <section class="section">
        <div class="container" style="max-width:800px;">
            ...
            <div class="form-wrapper" style="max-width:100%; margin:0;">
                ...
                <p>الرسائل تُحذف تلقائياً بعد <b>{{ message_ttl_days }} {{ ttl_unit(message_ttl_days) }}</b> من إرسالها.</p>
                ...
                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.download|safe }} كيف أنزّل الصورة؟</h3>
                <p>من صفحة "رسائلي"، إذا كانت الرسالة تحتوي على صور، يظهر زر <b>تنزيل الصورة</b>. كما يمكنك الضغط على أي صورة لعرضها بالحجم الحقيقي وتنزيلها.</p>
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("✓ تطبيق خليك واضح v8.0 يعمل على:")
    print("   🌐 http://localhost:5000")
    print("=" * 60)
    print("✨ الميزات في v8.0:")
    print("   ⏱️  عد تنازلي 5 ثوانٍ عند إرسال الرسائل")
    print("   ⏱️  عد تنازلي 3 ثوانٍ عند الرد على الرسائل")
    print("   🗑️  حذف تلقائي للرسائل بعد يوم واحد")
