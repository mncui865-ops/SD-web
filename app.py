...
النسخة النهائية v4.0 - بدون حظر/متابعين + قسم رسائلنا
============================================================
"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .pub-user-nick { font-size: 11px; color: #d97706; font-weight: 700; }
        .pub-arrow {
        ...
        .reveal-choice .rc-icon svg { width: 22px; height: 22px; }
...
def public_messages():
    """
    عرض جميع الرسائل المرسلة في الموقع (مشاهدة فقط).
    - إذا اختار المرسل "كشف البروفايل" → يظهر اسمه وصورته ورابط بروفايله.
    - إذا اختار المرسل "كشف اللقب فقط" → يظهر اللقب فقط (بدون بروفايل).
    - إذا اختار المرسل "مجهول" → يظهر "مجهول".
    """
    public_msgs = []
                ...
                <div class="pub-card" data-tags="{{ m.reveal_type }}">
                    <div class="pub-header">
                        ...
                        {% elif m.reveal_type == 'nickname' and m.sender %}
                            <div class="pub-avatar-anon" title="لقب فقط">🎭</div>
                        {% else %}
                            <div class="pub-avatar-anon" title="مجهول">👤</div>
                        {% endif %}
                        ...
                        <div class="pub-users">
                            ...
                            <div class="pub-user">
                                ...
                                {% elif m.reveal_type == 'nickname' and m.sender %}
                                    <div class="pub-user-avatar-anon">🎭</div>
                                    <div class="pub-user-info">
...
def _handle_send_message(username, user):
    ...
    <div class="form-wrapper" style="max-width:560px;">
        ...
        <form method="POST" id="send-form">
            ...
            <div class="form-group">
                <label>{{ icons.messages|safe }} نص الرسالة</label>
...
if __name__ == '__main__':
    print("=" * 60)
    print("✓ تطبيق خليك واضح v4.0 يعمل على:")
    print("   🌐 http://localhost:5000")
    print("=" * 60)
    print("✨ الميزات:")
    print("   • تم حذف: الحظر/إلغاء الحظر + المتابعون")
    print("   • تمت إضافة: قسم 'رسائلنا' العام (مشاهدة فقط)")
    print("   • خيار إجباري عند الإرسال: كشف البروفايل / كشف اللقب / مجهول")
    print("   • 6 صور في القائمة الرئيسية")
    print("=" * 60)
