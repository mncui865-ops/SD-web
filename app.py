...
النسخة النهائية v5.0
- عد تنازلي عند الإرسال
...
# ============================================================
# 🔔 رابط الملف الصوتي للإشعارات (عدّله من هنا)
# ============================================================
NOTIFICATION_SOUND_URL = "https://actions.google.com/sounds/v1/alarms/beep_short.ogg"
# يمكنك استبداله بأي رابط mp3/ogg آخر مثل:
# NOTIFICATION_SOUND_URL = "https://cdn.pixabay.com/download/audio/2022/03/15/audio_c8c8e8c5a3.mp3"
...
def start_cleanup_thread():
    """خيط في الخلفية يحذف الرسائل القديمة كل ساعة"""
    def worker():
        while True:
            try:
                removed = cleanup_old_messages()
                if removed > 0:
                    print(f"🗑️  تم حذف {removed} رسالة قديمة (أقدم من 7 أيام)")
...
messages_db = {}
ids_db = {}
notifications_db = {}   # {username: [notifications]}
...
def add_notification(username, notif_type, title, body, link='', icon=''):
    ...
    notif = {
        'id': secrets.token_hex(6),
        'type': notif_type,   # 'message', 'reply', 'system'
        'title': title,
        'body': body,
        'link': link,
        'icon': icon,
...
def validate_image_data_uri(data_uri):
    ...
    if not any(data_uri.startswith(p) for p in valid_prefixes):
        return '', 'صيغة الصورة غير مسموحة (المسموح: JPG, PNG, WEBP, GIF)'

    if len(data_uri) > 5_500_000:
        size_mb = len(data_uri) / (1024 * 1024)
        return '', f'حجم الصورة كبير جداً ({size_mb:.1f}MB). الحد الأقصى 5.5MB'
...
def inject_globals():
    ...
    unread = get_unread_count(session.get('user_username', '')) if session.get('user_username') else 0
    return {
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .lightbox .close-lb {
            ...
            cursor: pointer; transition: 0.25s; z-index: 10000;
            display: flex; align-items: center; justify-content: center;
        }
        .lightbox .close-lb:hover { background: rgba(231, 76, 60, 0.85); transform: scale(1.1) rotate(90deg); }
        ...
        .pub-avatar { width: 52px; height: 52px; border-radius: 50%; object-fit: cover; border: 3px solid #fff; box-shadow: 0 3px 10px rgba(0,0,0,0.1); flex-shrink: 0; background: #eee; }
        ...
        .pub-filters { display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; margin-bottom: 24px; }
        .pub-filter-btn { padding: 8px 18px; border-radius: 20px; background: #fff; border: 2px solid #eef0f4; font-size: 13px; font-weight: 700; color: #666; cursor: pointer; transition: 0.25s; font-family: inherit; display: inline-flex; align-items: center; gap: 6px; }
        .pub-filter-btn:hover { border-color: #14b8a6; color: #0d9488; transform: translateY(-2px); }
        .pub-filter-btn.active { background: linear-gradient(135deg, #14b8a6, #0d9488); color: #fff; border-color: #0d9488; box-shadow: 0 4px 14px rgba(20,184,166,0.35); }
        .pub-filter-btn svg { width: 14px; height: 14px; }

        .reveal-choices { display: grid; gap: 10px; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); margin-bottom: 18px; }
        .reveal-choice { position: relative; padding: 16px 14px; border: 2px solid #eef0f4; border-radius: 14px; background: #fafbfd; cursor: pointer; transition: 0.25s; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; }
        .reveal-choice:hover { border-color: #b95cff; background: #f9f5ff; transform: translateY(-2px); }
        .reveal-choice input[type="radio"] { position: absolute; opacity: 0; pointer-events: none; }
        .reveal-choice .rc-icon { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #fff; flex-shrink: 0; }
        .reveal-choice .rc-icon svg { width: 22px; height: 22px; stroke: #fff; }
        .reveal-choice .rc-title { font-size: 14px; font-weight: 800; color: #2d2f31; }
        .reveal-choice .rc-desc { font-size: 11px; color: #888; line-height: 1.4; }
        .reveal-choice.selected { border-color: #b95cff; background: linear-gradient(135deg, rgba(185,92,255,0.08), rgba(20,184,166,0.08)); box-shadow: 0 6px 20px rgba(185,92,255,0.2); }
        ...
        .notif-time { font-size: 11px; color: #999; margin-top: 6px; display: flex; align-items: center; gap: 4px; }
    ...
    <nav class="navbar">
        <div class="nav-container">
            ...
            <ul class="nav-menu">
                ...
                {% endif %}
                <li><a href="{{ url_for('help_page') }}" class="nav-btn-help" style="background: linear-gradient(135deg, #14b8a6, #0d9488);">
...
<section class="section" style="padding-top:20px;">
    <div class="container">
        ...
        <p style="text-align:center; color:#888; font-size:14px; margin-bottom:24px;">
            آخر {{ recent_public_messages|length }} رسائل — تُحذف تلقائياً بعد {{ message_ttl_days }} أيام
        </p>
        ...
        <div class="pub-timeline" style="max-width:780px; margin:0 auto;">
            ...
            <div class="pub-card" data-tags="{{ m.reveal_type }}">
                ...
                <div class="pub-images-grid">
                    {% for img in m.images %}
                    <img src="{{ img }}" alt="" onclick="openLightbox(this.src)">
                    {% endfor %}
                </div>
                {% endif %}
...
def reply_message(msg_id):
    ...
    for m in messages_db.get(username, []):
        if m['id'] == msg_id:
            m['replied'] = True
            m['reply'] = reply_text
            # إشعار للمرسل إذا كان معروفاً
            if m.get('sender') and m['sender'] in users_db:
                add_notification(
                    m['sender'], 'reply',
                    '💬 تم الرد على رسالتك',
                    f'@{username}: {reply_text[:60]}',
    ...
    for recipient_username, msgs in messages_db.items():
        ...
        for m in msgs:
            ...
            public_msgs.append({
                ...
                'reply': m.get('reply'),
                'reveal_type': reveal_type,
                'days_left': m.get('days_left', MESSAGE_TTL_DAYS),
                'sender': sender_data,
                'recipient': {
                    'username': recipient_username,
                    'name': recipient['name'],
        ...
        <div class="container" style="max-width:880px;">
            ...
            <div class="pub-timeline">
                ...
                <div class="pub-card" data-tags="{{ m.reveal_type }}">
                    ...
                    <div class="pub-footer">
                        <div class="pub-date">
                            {{ icons.clock|safe }} {{ m.created_at }} (الخرطوم)
                        </div>
                        <div class="pub-tags">
                            {% if m.reveal_type == 'profile' %}
...
def _self_send_blocked(username, user):
    """صفحة تظهر عند محاولة الإرسال لنفسك"""
    content = render_template_string(r"""
    <section class="section">
...
def _handle_send_message(username, user):
    if request.method == 'POST':
        ...
        # ✅ منع الإرسال لنفسك (تحقق إضافي عند POST)
        ...
        # 🔔 إشعار للمستلم
        sender_label = 'مجهول'
        if reveal_type == 'profile' and sender_username:
            sender_label = users_db.get(sender_username, {}).get('name', 'مستخدم')
        elif reveal_type == 'nickname' and sender_username:
            sender_label = users_db.get(sender_username, {}).get('nickname', 'مستخدم')

        preview = content_text[:60] if content_text else '[صورة]'
        add_notification(
            username, 'message',
            '📩 وصلتك رسالة جديدة!',
            f'من {sender_label}: {preview}',
            link='/messages',
        )
    ...
    // ✅ العد التنازلي قبل الإرسال
    function validateAndStartCountdown(e) {
        e.preventDefault();
        const form = e.target;

        // تحقق من اختيار الهوية
        const reveal = form.querySelector('input[name="reveal_type"]:checked');
        if (!reveal) {
            alert('يجب اختيار طريقة إظهار هويتك');
            return false;
        }

        // تحقق من وجود نص أو صور
            ...
            <div class="notif-list">
                ...
                <div class="notif-item {% if not n.read %}unread{% endif %}">
                    ...
                    <div class="notif-body">
                        ...
                        <div class="notif-time">{{ icons.clock|safe }} {{ n.created_at }} (الخرطوم)</div>
                        {% if n.link %}
    ...
    <section class="section">
        <div class="container" style="max-width:800px;">
            ...
            <div class="form-wrapper" style="max-width:100%; margin:0;">
                ...
                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.hourglass|safe }} ما هو العد التنازلي عند الإرسال؟</h3>
                <p>عند إرسال رسالة، يظهر عد تنازلي 5 ثوانٍ (مثل موقع صارحني) لإتاحة الفرصة لك للمراجعة قبل الإرسال النهائي.</p>

                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.trash|safe }} متى تُحذف الرسائل؟</h3>
                <p>الرسائل تُحذف تلقائياً بعد <b>{{ message_ttl_days }} أيام</b> من إرسالها. يمكنك رؤية عدد الأيام المتبقية لكل رسالة.</p>
                ...
                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.alert|safe }} لماذا لا يمكنني الإرسال لنفسي؟</h3>
                <p>عندما تدخل إلى رابطك الخاص، تظهر لك رسالة "لا يمكنك الإرسال لنفسك" — هذا لحماية خصوصيتك. شارك الرابط مع الآخرين فقط.</p>

                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.bell|safe }} كيف تعمل الإشعارات؟</h3>
                <p>عند وصول رسالة جديدة، يصلك إشعار فوري مع صوت تنبيه (بعد أول تفاعل مع الصفحة). يمكنك رؤية جميع الإشعارات من صفحة "الإشعارات".</p>
                ...
                <p>من صفحة "إدارة الحسابات"، اضغط "إضافة حساب Google" وسجّل بحساب آخر. يمكنك حفظ حتى 5 حسابات والتبديل بينها.</p>
                ...
                <p>عند إرسال رسالة، يمكنك إرفاق حتى 5 صور. الصور تُضغط تلقائياً لتناسب الحجم المسموح.</p>
            </div>
        </div>
    </section>
    """)
...
if __name__ == '__main__':
    # بدء خيط الحذف التلقائي
    start_cleanup_thread()

    print("=" * 60)
    print("✓ تطبيق خليك واضح v5.0 يعمل على:")
    print("   🌐 http://localhost:5000")
    print("=" * 60)
    print("✨ الميزات الجديدة في v5.0:")
