...
النسخة النهائية v3.0 - بدون لوحة أدمن
============================================================
"""
...
# ⭐ الصور الافتراضية في القائمة الرئيسية
gallery_db = [
    {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-btn-block  { background: linear-gradient(135deg, #e74c3c, #c0392b); }
        ...
        .blocked-item { display: flex; align-items: center; gap: 12px; padding: 14px; background: #fff; border-radius: 14px; margin-bottom: 10px; border: 1px solid #eef0f4; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
        .blocked-item img { width: 46px; height: 46px; border-radius: 50%; object-fit: cover; }
        .blocked-item .blocked-info { flex: 1; }
        .blocked-item .blocked-name { font-weight: 700; color: #2d2f31; }
        .blocked-item .blocked-user { color: #888; font-size: 13px; direction: ltr; }
...
<body>
        ...
        <div class="nav-container">
            ...
            <ul class="nav-menu">
                ...
                {% if session.user_username %}
                    ...
                    <li><a href="{{ url_for('blocked_list') }}" class="nav-btn-block">
                        {{ icons.block|safe }} المحظورون
                    </a></li>
    ...
    function blockUser(username) {
        ...
        fetch('/block/' + encodeURIComponent(username), {
            ...
            if (data.status === 'succ') { alert('تم الحظر'); location.reload(); }
            else alert(data.message || 'خطأ');
        });
    }
    function unblockUser(username) {
        fetch('/unblock/' + encodeURIComponent(username), {
            method: 'POST',
            headers: {'X-CSRF-Token': CSRF_TOKEN, 'Content-Type': 'application/x-www-form-urlencoded'},
            body: '_csrf_token=' + encodeURIComponent(CSRF_TOKEN),
            credentials: 'same-origin'
        }).then(r => r.json()).then(data => {
            if (data.status === 'succ') { alert('تم إلغاء الحظر'); location.reload(); }
            else alert(data.message || 'خطأ');
        });
    }
    ...
    <div class="container">
        ...
        <div class="features-grid">
            ...
            <div class="feature-card">
                ...
                <h4>رد على الرسائل</h4>
                <p>رد مباشر على كل رسالة تستقبلها</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon" style="background: linear-gradient(135deg, rgba(20,184,166,0.15), rgba(13,148,136,0.15));">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#14b8a6" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                </div>
                <h4>رابط مخصص</h4>
                <p>اختر رابطك الخاص وعطّله وقت ما تريد</p>
            </div>
        </div>
    </div>
</section>
...
def index():
    ...
    content = render_template_string(INDEX_CONTENT, gallery=gallery_render)
...
def auth_callback():
    ...
    if not username:
        ...
        blocked_db[username] = set()
        followers_db.setdefault(username, set())
        is_new = True
    else:
        user = users_db[username]
    ...
    if follow_target and follow_target in users_db and follow_target != username:
        followers_db.setdefault(follow_target, set()).add(username)
...
def members_list():
    ...
    for username, user in users_db.items():
        members.append({
            ...
            'followers_count': len(followers_db.get(username, set())),
        })
                ...
                <div class="member-card">
                    ...
                    <div class="member-info">
                        ...
                        <div class="member-meta">
                            ...
                            <span>{{ icons.users|safe }} {{ m.followers_count }}</span>
                        </div>
...
def profile():
    ...
    blocked_count = len(blocked_db.get(username, set()))
    followers_count = len(followers_db.get(username, set()))
        ...
        <div class="container" style="max-width:820px;">
            ...
            <div class="profile-header">
                ...
                <div class="profile-actions">
                    ...
                    <a href="{{ url_for('blocked_list') }}" class="btn btn-dark btn-sm">{{ icons.block|safe }} المحظورون ({{ blocked_count }})</a>
                </div>
            </div>
            <div class="stats-grid">
                ...
                <div class="stat-card"><div class="num">{{ followers_count }}</div><div class="lbl">{{ icons.users|safe }} المتابعون</div></div>
    ...
    """,
        ...
        blocked_count=blocked_count, followers_count=followers_count,
    )
...
def view_profile(username):
    ...
    if session.get('user_username'):
        is_blocked = username in blocked_db.get(session['user_username'], set())
    ...
    <section class="section">
        <div class="container" style="max-width:760px;">
            <div class="profile-header">
                ...
                {% endif %}
                <div class="profile-actions">
                    <a href="{{ url_for('send_message', username=user.username) }}" class="btn btn-berry btn-sm">{{ icons.send|safe }} أرسل رسالة مجهولة</a>
                    {% if session.user_username and session.user_username != user.username %}
                        {% if is_blocked %}
                            <button class="btn btn-dark btn-sm" onclick="unblockUser('{{ user.username }}')">{{ icons.eye|safe }} إلغاء الحظر</button>
                        {% else %}
                            <button class="btn btn-primary btn-sm" onclick="blockUser('{{ user.username }}')">{{ icons.block|safe }} حظر</button>
                        {% endif %}
                    {% elif not session.user_username %}
                        <a href="{{ url_for('signin', follow=user.username) }}" class="btn btn-blue btn-sm">{{ icons.accounts|safe }} متابعة</a>
                    {% endif %}
                </div>
            </div>
            <div class="stats-grid">
    ...
    """,
        user=user, avatar_url=avatar_url,
        is_blocked=is_blocked, messages_count=msgs_count,
        followers_count=followers_count, days_active=days_active,
    )
    return render_page(content, page_title=f'@{username}')
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        if new_username != username:
            ...
            blocked_db[new_username] = blocked_db.pop(old_username, set())
            followers_db[new_username] = followers_db.pop(old_username, set())
...
def blocked_list():
    username = session['user_username']
    blocked_names = blocked_db.get(username, set())
    blocked_users = []
    for bn in blocked_names:
        if bn in users_db:
            ...
            blocked_users.append({
                'username': bn,
                'name': u['name'],
                'avatar': get_user_avatar(u),
            })

    content = render_template_string(r"""
    <section class="section">
        <div class="container" style="max-width:700px;">
            <h2 class="section-title">{{ icons.block|safe }} المحظورون</h2>
            {% if blocked_users %}
                {% for bu in blocked_users %}
                <div class="blocked-item">
                    <img src="{{ bu.avatar }}" alt="{{ bu.username }}">
                    <div class="blocked-info">
                        <div class="blocked-name">{{ bu.name }}</div>
                        <div class="blocked-user">@{{ bu.username }}</div>
                    </div>
                    <button class="btn btn-primary btn-sm" onclick="unblockUser('{{ bu.username }}')">{{ icons.eye|safe }} إلغاء الحظر</button>
                </div>
                {% endfor %}
            {% else %}
                <div class="empty">
            ...
            <div style="text-align:center; margin-top:24px;">
                <a href="{{ url_for('profile') }}" class="btn btn-secondary">{{ icons.arrow_left|safe }} رجوع</a>
            </div>
        </div>
    </section>
    """, blocked_users=blocked_users)
    return render_page(content, page_title='المحظورون')


@app.route('/block/<username>', methods=['POST'])
@login_required
def block_user(username):
    me = session['user_username']
    if not validate_username(username):
        return jsonify({'status': 'error', 'message': 'اسم غير صالح'})
    if username == me:
        return jsonify({'status': 'error', 'message': 'لا يمكنك حظر نفسك'})
    if username not in users_db:
        return jsonify({'status': 'error', 'message': 'غير موجود'})
    blocked_db.setdefault(me, set()).add(username)
    return jsonify({'status': 'succ'})
...
def unblock_user(username):
    me = session['user_username']
    if me in blocked_db:
        blocked_db[me].discard(username)
    return jsonify({'status': 'succ'})
...
def _handle_send_message(username, user):
    ...
    if session.get('user_username'):
        is_blocked_by_owner = session['user_username'] in blocked_db.get(username, set())

    if request.method == 'POST':
        if is_blocked_by_owner:
            flash('لا يمكنك إرسال رسائل لهذا المستخدم', 'error')
            return redirect(request.path)
        content_text = sanitize_text(request.form.get('content') or '', 1000)
        if not content_text:
            flash('الرجاء كتابة رسالة', 'warning')
        else:
            messages_db.setdefault(username, []).append({
                'id': secrets.token_hex(6),
    ...
    <div class="form-wrapper">
        ...
        <h2 class="form-title">{{ icons.send|safe }} أرسل رسالة مجهولة</h2>
        <p class="form-sub">إلى @{{ username }} — هويتك سرية</p>
        <form method="POST">
            <input type="hidden" name="_csrf_token" value="{{ csrf_token() }}">
            <div class="form-group">
                <textarea name="content" placeholder="اكتب رسالتك..." required maxlength="1000" style="min-height:140px;"></textarea>
            </div>
...
def follow_user(username):
    if not validate_username(username) or username not in users_db:
        return jsonify({'status': 'error', 'message': 'غير موجود'})
    if 'user_username' not in session:
        return jsonify({'status': 'need_login', 'target': username})
    me = session['user_username']
    if me == username:
        return jsonify({'status': 'error', 'message': 'لا يمكنك متابعة نفسك'})
    followers_db.setdefault(username, set()).add(me)
    return jsonify({'status': 'succ'})
    ...
    <section class="section">
        <div class="container" style="max-width:800px;">
            ...
            <div class="form-wrapper" style="max-width:100%; margin:0;">
                ...
                <h3 style="color:#b95cff; margin-top:20px; display:flex; align-items:center; gap:8px;">{{ icons.sparkle|safe }} ما مميزات خليك واضح؟</h3>
                <p>خصوصية كاملة، حماية متقدمة، رد على الرسائل، حظر المستخدمين المزعجين، لقب مميز، رابط مخصص، صورة شخصية قابلة للتغيير.</p>
            </div>
        </div>
    </section>
    """)
...
def too_large(e):
    return jsonify({'status': 'toolarge', 'message': 'حجم كبير جداً'}), 413
...
if __name__ == '__main__':
    print("=" * 60)
    print("✓ تطبيق خليك واضح يعمل على:")
